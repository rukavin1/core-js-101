...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const result = `${startBracket}${Math.min(a, b)}, ${Math.max(a, b)}${endBracket}`;
  return result;
}
...
function reverseInteger(num) {
  ...
  const res = strNum.split('').reverse().join('');
  return +res;
}
...
function getDigitalRoot(num) {
  let sum = num;
  while (sum > 9) {
    sum = Array.from(String(sum), Number).reduce((acc, curr) => acc + curr, 0);
  }
  return sum;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < directories[0].length; i += 1) {
    const currentDirectory = directories[0][i];
    if (directories.every((directory) => directory[i] === currentDirectory)) {
